...
void setup(){                                       //
   ...
   while(!mySD.begin(SD_CHIP_SELECT)){}             // инициируем работу с SD картой, ожидая завершения инициализации в цикле while (не даём войти в loop пока нет карты)
                                                    ...
                                                    //
void loop(){                                        //
   myFiles.load(0,0,160,128,"image_160x128_a.raw"); // выводим на дисплей картинку начиная с координаты 0,0 размером 160,128 из файла image_160x128_a.raw
   delay(2000);                                     // ждём 2 секунды
   myFiles.load(0,0,160,128,"image_160x128_b.raw"); // выводим на дисплей картинку начиная с координаты 0,0 размером 160,128 из файла image_160x128_b.raw
   delay(2000);                                     // ждём 2 секунды
}                                                   //
                                                    //
/* ВНИМАНИЕ!!!
